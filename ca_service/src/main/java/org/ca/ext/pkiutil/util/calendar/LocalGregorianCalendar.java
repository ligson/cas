...
import org.ca.ext.pkiutil.util.action.GetPropertyAction;
...
public class LocalGregorianCalendar extends BaseCalendar {
	...
	static LocalGregorianCalendar getLocalGregorianCalendar(String name) {
		...
		try {
			...
			calendarProps = (Properties) AccessController.doPrivileged(new PrivilegedExceptionAction<Properties>() {
		...
		/*
		 * String resourceDir = null; final String fname = resourceDir +
		 * File.separator + "calendars.properties"; try { calendarProps.load(new
		 * FileInputStream(fname)); } catch (Exception e) { throw new
		 * RuntimeException(e); }
		 */
		// Parse calendar.*.eras
	...
	public boolean validate(CalendarDate date) {
		...
		if (era != null) {
			if (!validateEra(era)) { return false; }
	...
	public boolean isLeapYear(Era era, int year) {
		if (era == null) { return isLeapYear(year); }
