...
/**
 * The <code>CalendarDate</code> class represents a specific instant in time by
 * calendar date and time fields that are multiple cycles in different time
 * unites. The semantics of each calendar field is given by a concrete calendar
 * system rather than this <code>CalendarDate</code> class that holds calendar
 * field values without interpreting them. Therefore, this class can be used to
 * represent an amount of time, such as 2 years and 3 months.
 * 
 * <p>
 * A <code>CalendarDate</code> instance can be created by calling the
 * <code>newCalendarDate</code> or <code>getCalendarDate</code> methods in
 * <code>CalendarSystem</code>. A <code>CalendarSystem</code> instance is
 * obtained by calling one of the factory methods in <code>CalendarSystem</code>
 * . Manipulations of calendar dates must be handled by the calendar system by
 * which <code>CalendarDate</code> instances have been created.
 * 
 * <p>
 * Some calendar fields can be modified through method calls. Any modification
 * of a calendar field brings the state of a <code>CalendarDate</code> to <I>not
 * normalized</I>. The normalization must be performed to make all the calendar
 * fields consistent with a calendar system.
 * 
 * <p>
 * The <code>protected</code> methods are intended to be used for implementing a
 * concrete calendar system, not for general use as an API.
 * 
 * @see CalendarSystem
...
public abstract class CalendarDate implements Cloneable {
	public final int FIELD_UNDEFINED = Integer.MIN_VALUE;
	...
	/**
	 * Sets the era of the date to the specified era. The default implementation
	 * of this method accepts any Era value, including <code>null</code>.
	 * 
	 * @exception NullPointerException
	 *                if the calendar system for this <code>CalendarDate</code>
	 *                requires eras and the specified era is null.
	 * @exception IllegalArgumentException
	 *                if the specified <code>era</code> is unknown to the
	 *                calendar system for this <code>CalendarDate</code>.
	 */
	public CalendarDate setEra(Era era) {
		if (this.era == era) { return this; }
		this.era = era;
		normalized = false;
		return this;
	}
	...
	/**
	 * Returns whether the year represented by this <code>CalendarDate</code> is
	 * a leap year. If leap years are not applicable to the calendar system,
	 * this method always returns <code>false</code>.
	 * 
	 * <p>
	 * If this <code>CalendarDate</code> hasn't been normalized,
	 * <code>false</code> is returned. The normalization must be performed to
	 * retrieve the correct leap year information.
	 * 
	 * @return <code>true</code> if this <code>CalendarDate</code> is normalized
	 *         and the year of this <code>CalendarDate</code> is a leap year, or
	...
	/**
	 * Returns the day of week value. If this CalendarDate is not normalized,
	 * {@link #FIELD_UNDEFINED} is returned.
	 * 
	...
	public long getTimeOfDay() {
		if (!isNormalized()) { return fraction = TIME_UNDEFINED; }
		return fraction;
	}
	...
	public boolean isDaylightTime() {
		if (isStandardTime()) { return false; }
	 ...
	 * <code>CalendarDate</code>. The time of the day fields are ignored for the
	 * comparison.
	 */
	...
	public boolean equals(Object obj) {
		...
		if (isNormalized() != that.isNormalized()) { return false; }
		boolean hasZone = zoneinfo != null;
		boolean thatHasZone = that.zoneinfo != null;
		if (hasZone != thatHasZone) { return false; }
		if (hasZone && !zoneinfo.equals(that.zoneinfo)) { return false; }
	...
	/**
	 * Converts calendar date values to a <code>String</code> in the following
	 * format.
	 * 
	 * <pre>
