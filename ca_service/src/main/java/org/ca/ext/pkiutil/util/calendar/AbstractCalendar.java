...
/**
 * The <code>AbstractCalendar</code> class provides a framework for implementing
 * a concrete calendar system.
 * 
 * <p>
 * <a name="fixed_date"></a><B>Fixed Date</B><br>
 * 
 * For implementing a concrete calendar system, each calendar must have the
 * common date numbering, starting from midnight the onset of Monday, January 1,
 * 1 (Gregorian). It is called a <I>fixed date</I> in this class. January 1, 1
 * (Gregorian) is fixed date 1. (See Nachum Dershowitz and Edward M. Reingold,
 * <I>CALENDRICAL CALCULATION The Millennium Edition</I>, Section 1.2 for
 * details.)
 * 
 * @since 1.5
 */
...
public abstract class AbstractCalendar extends CalendarSystem {
	...
	public Era getEra(String eraName) {
		if (eras != null) {
			for (int i = 0; i < eras.length; i++) {
				if (eras[i].equals(eraName)) { return eras[i]; }
			}
		}
		return null;
	}
	...
	public void setEra(CalendarDate date, String eraName) {
		if (eras == null) { return; // should report an error???
		}
		for (int i = 0; i < eras.length; i++) {
			Era e = eras[i];
	...
	public long getTime(CalendarDate date) {
		...
		TimeZone zi = date.getZone();
		if (zi != null) {
			if (date.isNormalized()) { return ms - date.getZoneOffset(); }
	...
	protected long getTimeOfDay(CalendarDate date) {
		...
		if (fraction != CalendarDate.TIME_UNDEFINED) { return fraction; }
	...
	/**
	 * Returns a date of the given day of week that is closest to and after the
	 * given fixed date.
	 * 
	 * @param fixedDate
	 *            the fixed date
	...
	public static long getDayOfWeekDateOnOrBefore(long fixedDate, int dayOfWeek) {
		...
		if (fd >= 0) { return fixedDate - (fd % 7); }
		return fixedDate - CalendarUtils.mod(fd, 7);
	}

	/**
	 * Returns the fixed date calculated with the specified calendar date. If
	 * the specified date is not normalized, its date fields are normalized.
	 * 
	 * @param date
	 *            a <code>CalendarDate</code> with which the fixed date is
	 *            calculated
	...
	/**
	 * Calculates calendar fields from the specified fixed date. This method
	 * stores the calculated calendar field values in the specified
	 * <code>CalendarDate</code>.
	 * 
	 * @param date
	 *            a <code>CalendarDate</code> to stored the calculated calendar
	 *            fields.
	 * @param fixedDate
	...
	int normalizeTime(CalendarDate date) {
		...
		if (fraction >= DAY_IN_MILLIS) {
			...
			if (days != 0) {
				fraction -= DAY_IN_MILLIS * days; // mod(fraction,
				// DAY_IN_MILLIS)
			}
		}
		if (days != 0) {
