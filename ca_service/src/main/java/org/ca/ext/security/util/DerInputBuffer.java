...
import java.util.Date; // import sun.util.calendar.CalendarDate;
// import sun.util.calendar.CalendarSystem;

import org.ca.ext.pkiutil.calendar.CalendarDate;
import org.ca.ext.pkiutil.calendar.CalendarSystem;
...
class DerInputBuffer extends ByteArrayInputStream implements Cloneable {
	...
	boolean equals(DerInputBuffer other) {
		...
		for (int i = 0; i < max; i++) {
			if (this.buf[this.pos + i] != other.buf[other.pos + i]) { return false; }
		}
		return true;
	}
	...
	BigInteger getBigInteger(int len, boolean makePositive) throws IOException {
		...
		if (len == 0) { throw new IOException("Invalid encoding: zero length Int value"); }
	...
	public int getInteger(int len) throws IOException {
		...
		if (result.compareTo(BigInteger.valueOf(Integer.MIN_VALUE)) < 0) { throw new IOException("Integer below minimum valid value"); }
		if (result.compareTo(BigInteger.valueOf(Integer.MAX_VALUE)) > 0) { throw new IOException("Integer exceeds maximum valid value"); }
	...
	public byte[] getBitString(int len) throws IOException {
		...
		if (len == 0) { throw new IOException("Invalid encoding: zero length bit string"); }

		int numOfPadBits = buf[pos];
		if ((numOfPadBits < 0) || (numOfPadBits > 7)) { throw new IOException("Invalid number of padding bits"); }
	...
	BitArray getUnalignedBitString() throws IOException {
		...
		if (unusedBits > 7) { throw new IOException("Invalid value for unused bits: " + unusedBits); }
