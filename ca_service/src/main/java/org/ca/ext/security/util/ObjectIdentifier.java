package org.ca.ext.security.util;
...
public class ObjectIdentifier implements Serializable {
	...
	private void checkValidOid(int[] values, int len) throws IOException {
		if (values == null || len < 2) { throw new IOException("ObjectIdentifier() -- " + "Must be at least two oid components "); }

		for (int i = 0; i < len; i++) {
			if (values[i] < 0) { throw new IOException("ObjectIdentifier() -- " + "oid component #" + (i + 1) + " must be non-negative "); }
		}

		if (values[0] > maxFirstComponent) { throw new IOException("ObjectIdentifier() -- " + "First oid component is invalid "); }

		if (values[0] < 2 && values[1] > maxSecondComponent) { throw new IOException("ObjectIdentifier() -- " + "Second oid component is invalid "); }
	}
	...
	public ObjectIdentifier(DerInputStream in) throws IOException {
		...
		if (bufferEnd < 0) throw new IOException("ObjectIdentifier() -- not enough data");
	...
	private void initFromEncoding(DerInputStream in, int bufferEnd) throws IOException {
		...
		for (components = new int[allocationQuantum], componentLen = 0; in.available() > bufferEnd;) {
			...
			if (component < 0) { throw new IOException("ObjectIdentifier() -- " + "component values must be nonnegative"); }
			if (first_subid) {
				int X, Y;
				...
				if (componentLen >= components.length) {
					...
					tmp_components = new int[components.length + allocationQuantum];
					System.arraycopy(components, 0, tmp_components, 0, components.length);
		 ...
		 */
		if (in.available() != bufferEnd) { throw new IOException("ObjectIdentifier() -- malformed input data"); }
	}
	...
	public boolean precedes(ObjectIdentifier other) {
		...
		if (other == this || componentLen < other.componentLen) return false;
		if (other.componentLen < componentLen) return true;

		// for each component, the lesser component goes first
		for (i = 0; i < componentLen; i++) {
			if (other.components[i] < components[i]) return true;
		}
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof ObjectIdentifier == false) { return false; }
		ObjectIdentifier other = (ObjectIdentifier) obj;
		if (componentLen != other.componentLen) { return false; }
