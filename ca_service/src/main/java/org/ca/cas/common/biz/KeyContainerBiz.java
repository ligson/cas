...
public class KeyContainerBiz implements InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (keyJksFile.exists() && keyJksFile.length() != 0) {
            ...
            keyStore.load(null, null);
            for (int i = 0; i < 20; i++) {
                KeyPair keyPair = KeyStoreUtil.genKeyPair("RSA", 2048);
                Certificate certificate = CertUtil.wrapToCertContainer(keyPair.getPublic());
                keyStore.setKeyEntry(IdUtils.randomIntString(), keyPair.getPrivate(), keyPwd.toCharArray(), new Certificate[]{certificate});
            }
            keyStore.store(new FileOutputStream(keyJksFile), keyPwd.toCharArray());
            logger.info("初始化密钥库完成.........");
        }
