...
public class GenSelfCACertBiz extends AbstractBiz<GenSelfCertRequestDto, GenSelfCertResponseDto> {
    ...
    public Boolean txnPreProcessing() {
        ...
        try {
            subjectPublicKeyInfo = new SubjectPublicKeyInfo(new AlgorithmIdentifier(AlgorithmId.get(container.getPublicKey().getAlgorithm()).getOID().toString()), container.getPublicKey().getEncoded());
        } catch (NoSuchAlgorithmException e) {
            e.printStackTrace();
            setFailureResult(CertFailEnum.E_BIZ_21004);
            return false;
        }
        X509Certificate certificate = makeCertBiz.genV3(subjectPublicKeyInfo, container.getPrivateKey(), subject, subject, new BigInteger(serialNum), notBefore, notAfter, extensions);
        if (certificate == null) {
            setFailureResult(CertFailEnum.E_BIZ_21004);
            return false;
        }
        ...
        offlineCertEntity.setReqBuf(csr);
